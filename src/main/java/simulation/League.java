...
public class League {
    ...
    public ArrayList<HeadCoach> coachFreeAgents;
    public ArrayList<HeadCoach> coachDatabase;
    ...
    public League(File saveFile, String namesCSV, String lastNamesCSV) {
        ...
        try {
            ...
            while ((line = bufferedReader.readLine()) != null && !line.equals("END_COACHES")) {
                String h[] = line.split("%");
                int cPres = 0;
                boolean cRetire = false;
                String c[] = h[0].split(",");
                if (h.length > 1) {
                    if (c.length > 21) cPres = Integer.parseInt(c[21]);
                    if (c.length > 22) cRetire = Boolean.parseBoolean(c[22]);
                    ...
                    coachFreeAgents.add(new HeadCoach(c[0], Integer.parseInt(c[1]), Integer.parseInt(c[2]), Integer.parseInt(c[3]), Integer.parseInt(c[4]), Integer.parseInt(c[5]), Integer.parseInt(c[6]), Integer.parseInt(c[7])
                            ...
                            , Integer.parseInt(c[16]), Integer.parseInt(c[17]), Integer.parseInt(c[18]), Integer.parseInt(c[19]), Integer.parseInt(c[20]), cPres, cRetire));
     ...
     * @param saveFile file that league is saved in
     */
    public League(File saveFile, String namesCSV, String lastNamesCSV, boolean fixSave, MainActivity main) {
        setupCommonInitalizers();
        setupNamesDB(namesCSV, lastNamesCSV);
    ...
    */
    ...
    public void coachCarousel() {
            ...
            for (int t = 0; t < teamList.size(); ++t) {
                if (teamList.get(t).HC.isEmpty() && coachStarList.get(i).ratOvr >= teamList.get(t).getMinCoachHireReq() && teamList.get(t).name != tmName && Math.random() > 0.66) {
                    if (teamList.get(t).teamPrestige > tmPres && teamList.get(t).confPrestige > cPres || teamList.get(t).teamPrestige > tmPres + 5 || teamList.get(t).confPrestige + 10 > cPres) {
                        ...
                        coachStarList.remove(teamList.get(t).HC.get(0));
                        break;
                    }
                }
            }
        ...
        for (int i = 0; i < coachFreeAgents.size(); ++i) {
            for (int t = 0; t < teamList.size(); ++t) {
                if (teamList.get(t).HC.isEmpty() && coachFreeAgents.get(i).ratOvr >= teamList.get(t).getMinCoachHireReq() && Math.random() < 0.60 && !coachFreeAgents.get(i).retired) {
    ...
    public void coachHiringSingleTeam(Team school) {
        ...
        for (int i = 0; i < coachStarList.size(); ++i) {
            ...
            if (coachStarList.get(i).ratOvr >= school.getMinCoachHireReq() && school.name != tmName && Math.random() > 0.60) {
                if (school.teamPrestige > tmPres && school.confPrestige > cPres || school.teamPrestige > tmPres + 5 || school.confPrestige + 10 > cPres) {
                    ...
                    coachStarList.remove(school.HC.get(0));
                    break;
                }
            }
        }
        ...
        if (school.HC.isEmpty()) {
            ...
            Collections.sort(coachFreeAgents, new CompCoachOvr());
            for (int i = 0; i < coachFreeAgents.size(); ++i) {
                if (school.HC.isEmpty() && coachFreeAgents.get(i).ratOvr >= school.getMinCoachHireReq() && Math.random() < 0.65 && !coachFreeAgents.get(i).retired) {
...
*/
                ...
                if (conferences.get(c).confTeams.size() < maxConfTeams && conferences.get(c).confTeams.size() >= conferences.get(c).minConfTeams) {
                    ...
                    for (int i = 0; i < promoteTeamList.size(); i++) {
                        if (promoteTeamList.get(i).teamPrestige > (conf.confRelegateMin * 1.2) && Math.random() < realignmentChance && Math.abs(promoteTeamList.get(i).location - conf.confTeams.get(0).location) < 2) {
                            ...
                            //break the news
                            newsStories.get(currentWeek + 2).add("No More Independence!>The " + conf.confName + " conference announced today they will be adding " + teamA.name + " to their conference next season! " + teamA.name + " used to be unaffiliated as an Independent.");
        ...
        if (advancedRealignment && Math.random() < confRealignmentChance && Math.random() < realignmentChance) {
            ...
            if (matches < teamsFCS.length) {
                ...
                if (conferences.get(indConf).confTeams.size() < indy.minConfTeams - 1) {
                    ...
                    //break the news
                    newsStories.get(currentWeek + 2).add("Lower Division School Promoted!>Today is a special day at " + FCS.name + ". They have been promoted to Division I College Football today, and will be listed as an Independent team!");
    ...
    public ArrayList<String> getTeamRankingsStr(int selection) {
        ...
        switch (selection) {
            ...
            case 2:
                ...
                for (int i = 0; i < teams.size(); ++i) {
                    t = teams.get(i);
                    rankings.add(t.getRankStr(i + 1) + "," + t.name + "," + t.teamSOS);
                }
                break;
            case 3:
    ...
    public void createCoachDatabase() {
        ...
        for (int i = 0; i < teamList.size(); ++i) {
            if (teamList.get(i).HC.size() > 0) coachDatabase.add(teamList.get(i).HC.get(0));
        }
        ...
        switch (selection) {
            case 0:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    ...
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.natchamp);
                }
                break;
            case 1:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.confchamp);
                }
                break;
            case 2:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.bowlwins);
                }
                break;
            case 3:
            ...
            case 5:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.awards);
                }
                break;
            case 6:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.confAward);
                }
                break;
            case 7:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.allamericans);
                }
                break;
            case 8:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.allconference);
                }
                break;
            case 9:
            ...
            case 10:
                ...
                for (int i = 0; i < coachDatabase.size(); ++i) {
                    c = coachDatabase.get(i);
                    rankings.add(getRankStr(i + 1) + "," + c.name + checkCoachStatus(c) + "," + c.cumulativePrestige);
                }
                break;
        }
        return rankings;
    ...
    private String checkCoachStatus(HeadCoach c) {
        String s = "";
        if(c.retired) s = " [R]";
